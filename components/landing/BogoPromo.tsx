 ...
 * Shows combined flash sale + BOGO when both active, otherwise individual promotions.
 */
export function BogoPromo({ className = "" }: BogoPromoProps) {
  const discountActive = isDiscountActive();
  const bogoActive = isBogoPeriod();

  // Show combined banner when BOTH promotions are active
  if (discountActive && bogoActive) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex flex-col items-center justify-center gap-1 text-center">
            ...
            <span className="text-xs sm:text-sm text-amber-100/80">
              + Create <span className="font-semibold text-yellow-200">2 stores</span> during our BOGO promotion!
            </span>

            {/* CTA */}
            <a
              href="/auth/signup"
              className="mt-2 inline-flex items-center gap-1.5 px-4 py-1.5 bg-white hover:bg-amber-50 text-amber-700 text-sm font-semibold rounded-full transition-colors"
            >
              Claim Deal
  ...
  // Flash sale only (after BOGO expires)
  if (discountActive) {
    const daysLeft = getDiscountDaysRemaining();
    const price = getStarterPriceDisplay();
    const savings = getDiscountSavingsDisplay();
  ...
  // BOGO only (after flash sale expires)
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4 text-center">
          ...
          >
            One price, two stores. Start now.
            <ArrowRight className="w-4 h-4" />
          </a>
        </div>
      </div>
